...
module.exports = (g) => {
  ...
  return describe('routes', () => {
    ...
    // })
    it('must not create new page, you are not webmaster', async () => {
      const res = await r.post('/testdomain2.cz').send({ path: 'new/created.yaml' })
    ...
    it('shall create new page', async () => {
      ...
      const res = await r.post('/testdomain.cz').send({ path: 'new/created.yaml' })
        .set('Authorization', 'Bearer f')
      res.status.should.equal(200)
      await wait(1500)
      const res2 = await data.get('/testdomain.cz/_service/routes.json')
